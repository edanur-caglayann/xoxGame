import {
  ...
  console.log("Game Id -> " + game_id_data_deserialize.game_id);
  }
  ...
  const create_game_acc = async(data:CreateGame) => {
  const game_id_data = await connection.getAccountInfo(game_id);
  const game_id_data_deserialize = deserialize(GameIdSchema, GameId, game_id_data!.data);
  ...
  const encoded = serialize(CreateGameSchema, data);
  const concat = Uint8Array.of(3, ...encoded);

  game_id_data_deserialize.game_id += 1;

  console.log("-> " + game_id_data_deserialize.game_id);

  const gamePda = PublicKey.findProgramAddressSync(
    [Buffer.from("game"), Buffer.from([game_id_data_deserialize.game_id])],
    program_id
  );
  ...
  const game_read = async(gameCounter: number) => {
    const gamePda = PublicKey.findProgramAddressSync(
  ...
  const join_game_acc = async (join_data:JoinGame) => {
    ...
    const gamePda = PublicKey.findProgramAddressSync(
      [Buffer.from("game"), Buffer.from([join_data.game_counter])],
      program_id
    );
  ...
  create_game.player_address = player1Wallet.publicKey.toBytes();

    // create_game_acc(create_game)
